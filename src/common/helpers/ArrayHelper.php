...
namespace ccheng\config\common\helpers;
...
class ArrayHelper extends BaseArrayHelper
{
     ...
     * @return array
     */
    public static function itemsMergeDropDown($models, $idField = 'id', $titleField = 'title', $treeStat = 1, $levelField = 'level')
    {
        $arr = [];
        foreach ($models as $k => $model) {
            $arr[] = [
                $idField => $model[$idField],
                $titleField => self::itemsLevel($model[$levelField], $models, $k, $treeStat) . " " . $model[$titleField],
            ];

            if (!empty($model['-'])) {
                $arr = ArrayHelper::merge($arr,
                    self::itemsMergeDropDown($model['-'], $idField, $titleField, $treeStat, $levelField));
            }
        }
    ...
    {
        if ($column === null || (is_string($column) && $column !== '')) {
            ...
            foreach ($array as $key => $value) {
                ...
                foreach ($column as $item) {
                    $data[$k][$item] = \yii\helpers\ArrayHelper::getValue($value, $item);
                }
            }

            return $data;
        } else {
            throw new UnprocessableEntityHttpException('column类型错误!');
        }
    }
    ...
    public static function intersect($arr1, $arr2)
    {
        ...
        $diff2 = array_intersect($arr2, $arr1);
        return array_merge($diff1, $diff2);
    }
}